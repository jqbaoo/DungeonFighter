...
namespace Kernal
{
    public class DialogDataManager
    {
        ...
        public bool LoadAllDialogData(List<DialogDataFormat> _dialogDataArray)
        {
            ...
            //把条件判断原为注释所示   if (_listAllDialogDataArray != null && _listAllDialogDataArray.Count == 0)
            if (_listAllDialogDataArray != null )
            {
        ...
        /// <summary>
        /// 得到下一条记录 true：输出合法数据  false：没有输出对话数据
        /// </summary>
        ...
        public bool GetNextDialogInfoRecoder(int _diaSectionNum, out e_DialogSide _diaSide, out string _strPersonName, out string _strDialogContent)
        {
            ...
            return GetDialogInfoRecoder(_diaSectionNum, out _diaSide, out _strPersonName, out _strDialogContent); ;
        }
