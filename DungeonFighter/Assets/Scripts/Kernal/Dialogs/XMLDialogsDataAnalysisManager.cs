...
namespace Kernal
{
    public class XMLDialogsDataAnalysisManager : MonoBehaviour
    {
        ...
        private static string _strXMLPath;                                          //XML文件路径
        private static string _strXMLRootNodeName;                                  //XML根节点名称
        /*常量定义*/
        ...
        /// <summary>
        /// 初始化加载XML对话数据 true初始化成功 false初始化失败
        /// </summary>
        /// <returns>
        /// true 初始化成功
        /// false 初始化失败
        /// </returns>
        public void InitXMLDialogsData()
        {
            //路径赋值
            _strXMLPath = KernalParameter.GetDialogConfigXMLPath();
            _strXMLRootNodeName = KernalParameter.GetDialogConfigXMLRootNodeName();            
            //加载文件
            //在Start()中自动加载            
        }
        ...
        IEnumerator ReadXMLConfigByWWW()
        {
            ...
            while (tmp_WWW.isDone)          //while (!tmp_WWW.isDone) 
            {
