/*
 * 
 * 项目：暗黑角斗场
 * 
 * Title: 公共层，游戏工具方法
 * 
 * Description:
 *      具体作用：
 *      1、数据持久化的操作(PlayerPrefs类)
 *      2、查找组件
 *      
 * Version: 1.0
 *
 * Author:何柱洲
 * 
*/

using UnityEngine;
using System.Collections;
using System;
//工具类，把一些常用的方法放在这个类里面

namespace Kernal
{
    public class GameTool : MonoBehaviour
    {
        //清理内存（在切换场景的时候调用）
        public static void ClearMemory()
        {
            //手动调用垃圾回收
            //调用垃圾回收是会消耗性能的，所以不能频繁去调用
            GC.Collect();
            //卸载内存中没用的资源
            Resources.UnloadUnusedAssets();
        }
        //数据持久化
        //判断内存里面是否包含某个键
        public static bool HasKey(string key)
        {
            return PlayerPrefs.HasKey(key);
        }
        //取值
        public static int GetInt(string key)
        {
            return PlayerPrefs.GetInt(key);
        }
        public static float GetFloat(string key)
        {
            return PlayerPrefs.GetFloat(key);
        }
        public static string GetString(string key)
        {
            return PlayerPrefs.GetString(key);
        }
        //存值
        public static void SetInt(string key, int value)
        {
            PlayerPrefs.SetInt(key, value);
        }
        public static void SetFloat(string key, float value)
        {
            PlayerPrefs.SetFloat(key, value);
        }
        public static void SetString(string key, string value)
        {
            PlayerPrefs.SetString(key, value);
        }
        //删除内存中指定的键值对
        public static void DeleteKey(string key)
        {
            PlayerPrefs.DeleteKey(key);
        }
        //删除所有的键值对
        public static void DeleteAll()
        {
            PlayerPrefs.DeleteAll();
        }
        //分割字符串
        public static string[] SplitString(string str, char c)
        {
            return str.Split(c);
        }
        //查找子物体(要避免子物体有重名)
        public static Transform FindTheChild(GameObject parent, string childName)
        {
            Transform child = parent.transform.Find(childName);
            if (child == null)
            {
                foreach (Transform item in parent.transform)
                {
                    Transform tran = FindTheChild(item.gameObject, childName);
                    if (tran != null)
                    {
                        return tran;
                    }
                }
            }
            return child;
        }
        public static T GetChildComponent<T>(GameObject parent, string childName) where T : Component
        {
            Transform searchTrans = FindTheChild(parent, childName);
            if (searchTrans != null)
            {
                return searchTrans.GetComponent<T>();
            }
            return null;
        }
        public static T AddChildComponent<T>(GameObject parent, string childName) where T : Component
        {
            Transform searchTras = FindTheChild(parent, childName);
            T component = searchTras.GetComponent<T>();
            if (component != null)
            {
                DestroyImmediate(component);
                return searchTras.gameObject.AddComponent<T>();
            }
            return searchTras.gameObject.AddComponent<T>();
        }
        public static void AddChildToParent(Transform parent, Transform child)
        {
            child.parent = parent;
            child.localPosition = Vector3.zero;
            child.localScale = Vector3.one;
        }
    }
}